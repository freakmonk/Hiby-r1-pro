...
#define SQL_LIST_CONTINUE \
    ...
    "b.total_duration_ms,b.track_count,COALESCE(b.fingerprint,'')," \
    "COALESCE(b.date_added,0),COALESCE(b.date_modified,0)," \
    "COALESCE(p.last_played_at,0),p.completed,COALESCE(p.completed_at,0)," \
    "p.playback_speed " \
...
#define SQL_LIST_FINISHED \
    ...
    "LEFT JOIN authors a ON a.author_id=b.author_id " \
    "LEFT JOIN series s ON s.series_id=b.series_id " \
    "WHERE p.completed=1 ORDER BY p.completed_at DESC"
...
int audiobook_db_open(const char *db_path, sqlite3 **db_out) {
    ...
    if (rc_schema != SQLITE_OK) {
        fprintf(stderr, "[library] schema bootstrap failed: %s", err ? err : "?");
        sqlite3_free(err);
        sqlite3_close(db);
        return -1;
    }
...
int audiobook_get_chapter_at(sqlite3 *db, int book_id, int64_t pos_ms, char *out_title, int max_len) {
    if (!db) return 0;
    const char *sql = "SELECT c.title FROM chapters c "
                      "JOIN tracks t ON c.track_id = t.track_id "
                      "WHERE t.book_id = ? AND ? >= c.start_ms AND ? < c.end_ms "
                      "ORDER BY t.disc_number, t.track_number, c.ordinal LIMIT 1";
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(db, sql, -1, &stmt, NULL);
    if (rc != SQLITE_OK) return 0;
    sqlite3_bind_int(stmt, 1, book_id);
    sqlite3_bind_int64(stmt, 2, pos_ms);
    sqlite3_bind_int64(stmt, 3, pos_ms);
    
    int found = 0;
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        const char *title = (const char *)sqlite3_column_text(stmt, 0);
        if (title) {
            strncpy(out_title, title, max_len - 1);
...
int audiobook_delete_progress(sqlite3 *db, int book_id) {
    if (!db) return -1;
    audiobook_book_t b;
    if (audiobook_get_book(db, book_id, &b) == 1) {
        char path[1024];
        
        // Delete .json sidecar
        snprintf(path, sizeof(path), "%s/%s.json", b.root_path, b.book_key);
        unlink(path);
        
        // Also check if .pos exists in .audiobook_pos and delete it just in case
        snprintf(path, sizeof(path), "/usr/data/mnt/sd_0/Audiobooks/.audiobook_pos/%s.pos", b.book_key);
        unlink(path);
    }

    const char *sql = "DELETE FROM progress WHERE book_id = ?";
    sqlite3_stmt *stmt;
    if (sqlite3_prepare_v2(db, sql, -1, &stmt, NULL) == SQLITE_OK) {
        sqlite3_bind_int(stmt, 1, book_id);
        sqlite3_step(stmt);
...
int audiobook_is_favorite(sqlite3 *db, int book_id) {
    sqlite3_stmt *stmt = NULL;
    if (sqlite3_prepare_v2(db, "SELECT 1 FROM favorites WHERE book_id=?", -1, &stmt, NULL) != SQLITE_OK) return 0;
    sqlite3_bind_int(stmt, 1, book_id);
    int res = (sqlite3_step(stmt) == SQLITE_ROW) ? 1 : 0;
    sqlite3_finalize(stmt);
    return res;
}

int audiobook_toggle_favorite(sqlite3 *db, int book_id, int *out_is_fav) {
    int res = 0;
    if (audiobook_is_favorite(db, book_id)) {
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(db, "DELETE FROM favorites WHERE book_id=?", -1, &stmt, NULL) == SQLITE_OK) {
            sqlite3_bind_int(stmt, 1, book_id);
            if (sqlite3_step(stmt) == SQLITE_DONE) res = 1;
            sqlite3_finalize(stmt);
        }
        if (res && out_is_fav) *out_is_fav = 0;
    } else {
        sqlite3_stmt *stmt = NULL;
        if (sqlite3_prepare_v2(db, "INSERT INTO favorites(book_id, created_at) VALUES(?, strftime('%s','now'))", -1, &stmt, NULL) == SQLITE_OK) {
            sqlite3_bind_int(stmt, 1, book_id);
            if (sqlite3_step(stmt) == SQLITE_DONE) res = 1;
            sqlite3_finalize(stmt);
        }
        if (res && out_is_fav) *out_is_fav = 1;
    }
    return res ? 0 : -1;
}

#define SQL_LIST_FAVORITES \
    "SELECT b.book_id,b.book_key,b.title,b.sort_title," \
    "COALESCE(a.display_name,''),COALESCE(b.narrator,'')," \
    "COALESCE(s.display_name,''),COALESCE(b.series_number,0)," \
    "b.root_path,COALESCE(b.cover_path,''),COALESCE(b.cover_cache_path,'')," \
    "b.total_duration_ms,b.track_count,COALESCE(b.fingerprint,'')," \
    "b.date_added,b.date_modified,b.last_played_at,b.completed," \
    "b.playback_speed,b.author_id,b.series_id " \
    "FROM favorites f " \
...
int audiobook_list_favorites(sqlite3 *db, int (*cb)(const audiobook_book_t *book, void *ctx), void *ctx) {
    return list_books_generic(db, SQL_LIST_FAVORITES, cb, ctx);
}
...
int audiobook_save_book_json(const char *root_path, const char *book_key,
    ...
    fprintf(f, "{");
    fprintf(f, "  \"book_id\": %d,", p->book_id);
    fprintf(f, "  \"book_key\": \"%s\",", book_key);
    fprintf(f, "  \"track_ordinal\": %d,", p->track_ordinal);
    fprintf(f, "  \"position_ms\": %lld,", (long long)p->position_ms);
    fprintf(f, "  \"total_book_elapsed_ms\": %lld,",
            (long long)p->total_book_elapsed_ms);
    fprintf(f, "  \"playback_speed\": %.2f,", p->playback_speed);
    fprintf(f, "  \"completed\": %d,", p->completed);
    fprintf(f, "  \"completed_at\": %lld,", (long long)p->completed_at);
    fprintf(f, "  \"protected_until_ms\": %lld,",
            (long long)p->protected_until_ms);
    fprintf(f, "  \"last_saved_at\": %lld", (long long)p->last_saved_at);
    fprintf(f, "}");
...
void audiobook_derive_book_key(const char *root_path, char *out, int out_len) {
    ...
    for (const char *p = root_path; *p; p++) {
        h ^= (unsigned char)*p;
        h *= 1099511628211ULL;
    }
...
void audiobook_normalize_utf8(char *s) {
    if (!s) return;
    char *r = s;
    char *w = s;
    while (*r) {
        if ((unsigned char)r[0] == 0xd0 && (unsigned char)r[1] == 0xb8 && 
            (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x86) {
            *w++ = 0xd0; *w++ = 0xb9; r += 4;
        } else if ((unsigned char)r[0] == 0xd0 && (unsigned char)r[1] == 0x98 && 
                   (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x86) {
            *w++ = 0xd0; *w++ = 0x99; r += 4;
        } else if ((unsigned char)r[0] == 0xd0 && (unsigned char)r[1] == 0xb5 && 
                   (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x88) {
            *w++ = 0xd1; *w++ = 0x91; r += 4;
        } else if ((unsigned char)r[0] == 0xd0 && (unsigned char)r[1] == 0x95 && 
                   (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x88) {
            *w++ = 0xd0; *w++ = 0x81; r += 4;
        } else if ((unsigned char)r[0] == 0xd1 && (unsigned char)r[1] == 0x96 && 
                   (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x88) {
            *w++ = 0xd1; *w++ = 0x97; r += 4;
        } else if ((unsigned char)r[0] == 0xd0 && (unsigned char)r[1] == 0x86 && 
                   (unsigned char)r[2] == 0xcc && (unsigned char)r[3] == 0x88) {
            *w++ = 0xd0; *w++ = 0x87; r += 4;
        } else {
            *w++ = *r++;
