extern volatile int g_scan_abort;
...
static void scan_progress_cb_func(int phase, int current, int total, const char *msg, void *ctx) {
    (void)ctx;
    pthread_mutex_lock(&g_scan.mu);
    g_scan.phase = phase;
    g_scan.current = current;
    g_scan.total = total;
    if (msg) { strncpy(g_scan.msg, msg, sizeof(g_scan.msg)-1); g_scan.msg[sizeof(g_scan.msg)-1] = '\0'; }
    else { g_scan.msg[0] = '\0'; }
...
static void *scan_worker_main(void *arg) {
    ...
    if (open_rc == 0) {
        rc = audiobook_scan_library(db, AUDIOBOOK_LIBRARY_ROOT, scan_progress_cb_func, NULL);
...
int ui_run(uint16_t *fb, int fb_fd) {
    ...
    while (ui->running) {
        ...
        if (ui->screen == SCREEN_LIST && ui->list_mode == LIST_BT_DEVICES) {
            static uint64_t last_bt_scan = 0;
            if (now - last_bt_scan > 2000) {
                last_bt_scan = now;
                rebuild_screen(ui);
            }
        }
        ...
        if ((now - last_pan) >= PAN_INTERVAL_MS) {
            ...
            if (screensize == 0) screensize = 480 * 800 * 2;
            
            extern uint16_t *g_fb_hw; // exported from main.c
            if (g_fb_hw && fb) {
                memcpy(g_fb_hw, fb, screensize);
            }
        }
#else
    ...
    close_input(ui);
    player_shutdown();   /* stop playback FIRST so user knows it's closing */
    /* wait for scan to commit partial results */
    scan_worker_join();
    // catalog_worker_join();
    free_render_cache(ui);
    cover_shutdown();
    audiobook_db_close(ui->db);
...
static void navigate_to(ui_state_t *ui, ui_screen_t screen,
    ...
    if (screen == SCREEN_LIST && mode == LIST_BT_DEVICES) {
        bt_scan_start();
    }
...
static int handle_list_longpress(ui_state_t *ui, int x, int y) {
    ...
    if (ui->list_mode == LIST_CONTINUE) {
        pthread_mutex_lock(&g_cache_lock);
        int book_id = -1;
        if (idx < ui->list_count) {
            book_id = ui->list_items[idx].book_id;
        }
        pthread_mutex_unlock(&g_cache_lock);
        
        if (book_id > 0) {
            audiobook_delete_progress(ui->db, book_id);
            rebuild_screen(ui);
            return 1;
        }
    }
...
static int ui_handle_longpress(ui_state_t *ui, int x, int y) {
    switch (ui->screen) {
        ...
        case SCREEN_LIST: return handle_list_longpress(ui, x, y);
        default: return 0;
    }
}
...
static void draw_home(ui_state_t *ui) {
    ...
    int home_cont, home_fin, home_total, home_fav;
    pthread_mutex_lock(&g_cache_lock);
    home_cont = ui->home_continue_n;
    home_fav = ui->home_favorites_n;
        ...
        if (i == HOME_CONTINUE) {
            ...
            if (count > 0) {
                ...
                render_text_right(r, RENDER_FB_W - 24, y + 22, buf,
                                  FONT_SCALE_2, COL_ACCENT);
            }
        }
        /* Show favorites count */
        else if (i == HOME_FAVORITES) {
            int count = home_fav;
            if (count > 0) {
                char buf[16];
                snprintf(buf, sizeof(buf), "%d", count);
    ...
    if (ui->refresh_scanning) {
        char smsg[256];
        pthread_mutex_lock(&g_scan.mu);
        if (g_scan.total > 0 && g_scan.phase == 2)
            snprintf(smsg, sizeof(smsg), "Scanning: %d / %d", g_scan.current, g_scan.total);
        else
            snprintf(smsg, sizeof(smsg), "Scanning: %s", g_scan.msg[0] ? g_scan.msg : "...");
        pthread_mutex_unlock(&g_scan.mu);
        render_text_centered(r, 0, RENDER_FB_H - FOOTER_H - 36, RENDER_FB_W,
                             smsg, FONT_SCALE_2, COL_WHITE);
...
static int handle_home_touch(ui_state_t *ui, int x, int y) {
    ...
    switch (idx) {
        ...
        case HOME_AUTHORS:
            ...
            break;
        case HOME_FAVORITES:
            navigate_to(ui, SCREEN_LIST, LIST_FAVORITES, 0);
            break;
        case HOME_FOLDERS:
        ...
        case HOME_FINISHED:
            ...
            break;
        case HOME_SETTINGS:
            navigate_to(ui, SCREEN_LIST, LIST_SETTINGS, 0);
            break;
        case HOME_BACK:
...
static void collect_list_books(ui_state_t *ui, list_ctx_t *lc) {
    switch (ui->list_mode) {
        case LIST_FAVORITES:
            audiobook_list_favorites(ui->db, list_collect_cb, lc); break;
        case LIST_CONTINUE:
        ...
        case LIST_SETTINGS: {
            if (lc->capacity < 3) {
                lc->capacity = 10;
                lc->items = realloc(lc->items, lc->capacity * sizeof(list_item_t));
            }
            list_item_t *item1 = &lc->items[lc->count++];
            item1->book_id = 0; item1->is_folder = 0;
            int on = bt_get_powered();
            snprintf(item1->title, sizeof(item1->title), "Bluetooth: %s", on ? "ON" : "OFF");
            item1->author[0] = 0; item1->duration_ms = 0; item1->completed = 0; item1->has_progress = 0; item1->elapsed_ms = 0;
            
            list_item_t *item2 = &lc->items[lc->count++];
            item2->book_id = 1; item2->is_folder = 0;
            snprintf(item2->title, sizeof(item2->title), "Scan BT Devices");
            item2->author[0] = 0; item2->duration_ms = 0; item2->completed = 0; item2->has_progress = 0; item2->elapsed_ms = 0;

            list_item_t *item3 = &lc->items[lc->count++];
            item3->book_id = 2; item3->is_folder = 0;
            snprintf(item3->title, sizeof(item3->title), "Refresh Library");
            item3->author[0] = 0; item3->duration_ms = 0; item3->completed = 0; item3->has_progress = 0; item3->elapsed_ms = 0;
            break;
        }
        case LIST_BT_DEVICES: {
            bt_device_t devs[20];
            int cnt = bt_get_devices(devs, 20);
            if (lc->capacity < cnt) {
                lc->capacity = cnt + 10;
                lc->items = realloc(lc->items, lc->capacity * sizeof(list_item_t));
            }
            for (int i=0; i<cnt; i++) {
                ...
                it->book_id = 1000 + i; it->is_folder = 0;
                snprintf(it->title, sizeof(it->title), "%s", devs[i].name);
                snprintf(it->author, sizeof(it->author), "%s %s", 
                    devs[i].paired ? (devs[i].connected ? "[Connected]" : "[Paired]") : "[New]", devs[i].mac);
                it->duration_ms = 0; it->completed = 0; it->has_progress = 0; it->elapsed_ms = 0;
            }
            break;
        }
        case LIST_FOLDERS: {
            ...
            if (sqlite3_prepare_v2(ui->db, sql, -1, &stmt, NULL) == SQLITE_OK) {
                ...
                while (sqlite3_step(stmt) == SQLITE_ROW) {
                    ...
                    const char *rest = "";
                    int is_immediate_book = 0;
                    if (strcmp(rpath, prefix) == 0) {
                        is_immediate_book = 1;
                    } else if (strncmp(rpath, prefix, plen) == 0 && rpath[plen] == '/') {
                        rest = rpath + plen + 1;
                        if (strchr(rest, '/') == NULL) {
                            /* No more slashes -> this is the book folder itself!
                             * Show it directly as a book here, don't make the user
                             * drill down one more empty level. */
                            is_immediate_book = 1;
                        }
                    }

                    if (is_immediate_book) {
...
static void rebuild_home(ui_state_t *ui) {
    ...
    int fav = audiobook_list_favorites(ui->db, NULL, NULL);
    pthread_mutex_lock(&g_cache_lock);
    ui->home_continue_n = cont;
    ui->home_favorites_n = fav;
...
static void rebuild_list(ui_state_t *ui) {
    int is_str = (ui->list_mode == LIST_AUTHORS);
    if (is_str) {
        strlist_ctx_t sc;
...
static void rebuild_current_book(ui_state_t *ui, int with_cover) {
    ...
    if (with_cover) {
        ...
        if (cov) {
            ...
            ui->cur_cover_ok = 1;
    ui->cur_is_fav = audiobook_is_favorite(ui->db, ui->current_book_id);
        } else {
            ui->cur_cover_ok = 0;
    ui->cur_is_fav = audiobook_is_favorite(ui->db, ui->current_book_id);
        }
...
static void draw_list(ui_state_t *ui) {
    ...
    switch (ui->list_mode) {
        ...
        case LIST_FAVORITES:        title = "Favorites"; break;
        ...
        case LIST_SETTINGS:      title = "Settings"; break;
        case LIST_BT_DEVICES:    title = "Bluetooth Devices"; break;
    }
    ...
    if (ui->list_is_strlist) {
        ...
        /* Title bar (opaque overlay) */
        render_fill_rect(r, 0, 0, RENDER_FB_W, TITLE_BAR_H, COL_BG);
        render_text(r, 18, 16, "Back", FONT_SCALE_1, COL_GRAY_LT);
        render_text_right(r, RENDER_FB_W - 18, 16, title, FONT_SCALE_2, COL_WHITE);
        render_draw_hline(r, 0, TITLE_BAR_H - 1, RENDER_FB_W, COL_DIVIDER);

        /* Footer (opaque overlay) */
        render_fill_rect(r, 0, RENDER_FB_H - FOOTER_H, RENDER_FB_W, FOOTER_H, COL_BG);
        char footer[64];
    ...
    for (int i = 0; i < count; i++) {
        ...
        int is_sys = (ui->list_mode == LIST_SETTINGS || ui->list_mode == LIST_BT_DEVICES);
        const uint16_t *thumb = is_sys ? NULL : cover_thumb_cached(bid);
        if (!is_sys && !thumb && ui->thumb_warm_target == 0 && !thumb_is_failed(ui, bid))
        ...
        /* Second line: elapsed / duration */
        char line2[256];
        line2[0] = '\0';
        
        if (ui->list_mode == LIST_SETTINGS) {
            // No second line for Settings
        } else if (ui->list_mode == LIST_BT_DEVICES) {
            snprintf(line2, sizeof(line2), "%s", items[i].author); // Contains MAC/Status
        } else {
            char dur[32];
        ...
        if (line2[0]) {
            ...
            render_text(r, text_x, item_top + 30, title_buf, FONT_SCALE_2, COL_WHITE);
        }
    ...
    render_fill_rect(r, 0, RENDER_FB_H - FOOTER_H, RENDER_FB_W, FOOTER_H, COL_BG);
    char footer[64];
    if (ui->list_mode == LIST_FOLDERS || ui->list_mode == LIST_SETTINGS)
        snprintf(footer, sizeof(footer), "%d items", count);
    else if (ui->list_mode == LIST_BT_DEVICES)
        snprintf(footer, sizeof(footer), "%d devices", count);
    else
...
static int handle_list_touch(ui_state_t *ui, int x, int y) {
    ...
    if (ui->list_mode == LIST_SETTINGS) {
        if (idx == 0) {
            ...
            navigate_to(ui, SCREEN_LIST, LIST_BT_DEVICES, 0);
        } else if (idx == 2) {
            ...
            navigate_back(ui); // Return to home screen to see refresh status
        }
        return 1;
    }
    
    if (ui->list_mode == LIST_BT_DEVICES) {
        pthread_mutex_lock(&g_cache_lock);
        list_item_t sel; int have = 0;
        if (idx < ui->list_count) { sel = ui->list_items[idx]; have = 1; }
        pthread_mutex_unlock(&g_cache_lock);
        if (have) {
            char mac[32] = {0};
            char *mac_ptr = strrchr(sel.author, ' ');
            if (mac_ptr) {
                strncpy(mac, mac_ptr + 1, sizeof(mac)-1);
                bt_connect(mac);
                rebuild_screen(ui);
            }
        }
...
static void draw_detail(ui_state_t *ui) {
    ...
    /* Row 1: Play, Chapters */
    render_fill_rect(r, 16, btn_y, btn_w, 64, COL_ACCENT);
    ...
    render_text_centered(r, 16, btn_y + 18, btn_w, "Bookmarks", FONT_SCALE_4,
                         COL_WHITE);
    if (ui->cur_is_fav) {
        render_fill_rect(r, 32 + btn_w, btn_y, btn_w, 64, COL_ACCENT);
        render_text_centered(r, 32 + btn_w, btn_y + 18, btn_w, "unFav", FONT_SCALE_4, COL_WHITE);
    } else {
        render_fill_rect(r, 32 + btn_w, btn_y, btn_w, 64, COL_GRAY);
        render_text_centered(r, 32 + btn_w, btn_y + 18, btn_w, "Fav", FONT_SCALE_4, COL_WHITE);
    }
...
static int handle_detail_touch(ui_state_t *ui, int x, int y) {
    ...
    if (y >= btn_y && y < btn_y + 64) {
        if (x < 16 + btn_w) {
            ...
            audiobook_toggle_favorite(ui->db, ui->current_book_id, &ui->cur_is_fav);
            return 1;
        }
    }
    return 0;
...
static void draw_now_playing(ui_state_t *ui) {
    ...
    char ch_label[256];
    if (audiobook_get_chapter_at(ui->db, ui->current_book_id, pos_ms, ch_label, sizeof(ch_label))) {
        render_text(r, 16, y, ch_label, FONT_SCALE_1, COL_GRAY_LT);
    } else {
    ...
    render_fill_rect(r, gap * 2 + ctrl_w, ctrl_y, ctrl_w, 60, COL_ACCENT);
...
static void draw_bookmarks(ui_state_t *ui) {
    ...
    render_fill_rect(r, 0, 0, RENDER_FB_W, TITLE_BAR_H, COL_BG);
    render_text(r, 18, 16, "Back", FONT_SCALE_1, COL_GRAY_LT);
    render_text_right(r, RENDER_FB_W - 18, 16, "Bookmarks", FONT_SCALE_2, COL_WHITE);
    render_draw_hline(r, 0, TITLE_BAR_H - 1, RENDER_FB_W, COL_DIVIDER);
...
static void draw_chapters(ui_state_t *ui) {
    ...
    render_fill_rect(r, 0, 0, RENDER_FB_W, TITLE_BAR_H, COL_BG);
    render_text(r, 18, 16, "Back", FONT_SCALE_1, COL_GRAY_LT);
    render_text_right(r, RENDER_FB_W - 18, 16, "Chapters", FONT_SCALE_2, COL_WHITE);
    render_draw_hline(r, 0, TITLE_BAR_H - 1, RENDER_FB_W, COL_DIVIDER);
