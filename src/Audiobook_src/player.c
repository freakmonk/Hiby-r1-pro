...
static struct {
    ...
    int64_t sleep_deadline_ms;
    int64_t idle_since_ms;   /* monotonic ms deadline, 0 = no sleep timer */
...
static void mixer_init(void) {
    ...
    if (sv < 0) sv = stock_volume_load();
    if (sv < 0) sv = 50;
    g_pl.volume_pct = sv;
    g_pl.wired_volume_pct = g_pl.volume_pct;
    g_pl.mixer_ok = 1;
    mix_apply(); /* Apply the loaded volume to the hardware DAC immediately on startup */
    ...
    if (start_ms < 0) {
        ...
        if (pos_load_sd(book_id, &sd_ord, &sd_pos, &sd_book, &sd_done) > 0) {
            if (sd_done) {
                start_idx = 0;
                into = 0;
                plog("resume(SD) book %d was finished, restarting", book_id);
            } else {
                start_idx = sd_ord - 1;
            ...
            if (audiobook_get_progress(g_pl.db, book_id, &p) > 0) {
                if (p.completed) {
                    start_idx = 0;
                    into = 0;
                    plog("resume(db) book %d was finished, restarting", book_id);
                } else {
...
static void cmd_resume(void) {
    ...
    /* Un-stick fallback flags on manual resume so the user can retry BT */
    g_pl.force_wired = 0;
    g_pl.bt_fell_back = 0;
    
    if (g_pl.pcm && g_pl.output == OUT_WIRED) {
        if (bt_detect()) {
            plog("RESUME detected new BT device, switching before play");
            setup_alsa(g_pl.rate, g_pl.channels);
            mix_apply();
        }
    }
...
static void *player_thread(void *arg) {
    ...
    while (g_pl.running) {
        ...
        if (g_pl.state == PLAYER_PLAYING && g_pl.track_open && g_pl.pcm) {
            ...
            pthread_mutex_unlock(&g_pl.mu);
            if (sleep_fire) {
                plog("sleep timer expired -> power off");
                save_progress(0, 1);
                system("poweroff");
                exit(0);
            } else {
                g_pl.idle_since_ms = mono_ms(); // reset idle while playing
                
                static uint64_t last_bt_check = 0;
                if (g_pl.output == OUT_WIRED) {
                    uint64_t now = mono_ms();
                    if (now - last_bt_check > 3000) {
                        last_bt_check = now;
                        int bt_avail = bt_detect();
                        if (g_pl.force_wired) {
                            if (!bt_avail) {
                                plog("BT device disconnected, clearing force_wired");
                                g_pl.force_wired = 0;
                                g_pl.bt_fell_back = 0;
                            }
                        } else {
                            if (bt_avail) {
                                plog("BT device connected during playback, switching to BT!");
                                setup_alsa(g_pl.rate, g_pl.channels);
                                mix_apply();
                            }
                        }
            ...
            if (mono_ms() - g_pl.idle_since_ms > 15LL * 60LL * 1000LL) {
                plog("idle timeout (15 min) -> power off");
                save_progress(0, 1);
                system("poweroff");
                exit(0);
            }
...
void player_reset_idle(void) {
    if (g_pl.thread_alive) g_pl.idle_since_ms = mono_ms();
}
