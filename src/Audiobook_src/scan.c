...
int audiobook_scan_library(sqlite3 *db, const char *root_path,
    ...
    for (int i = 0; i < book_count; i++) {
        ...
        /* Normalize macOS NFD Cyrillic to NFC */
        audiobook_normalize_utf8(path_author);
        audiobook_normalize_utf8(path_series);
        audiobook_normalize_utf8(title);
        audiobook_normalize_utf8(sort_title);
        audiobook_normalize_utf8(tag_artist);
        audiobook_normalize_utf8(tag_composer);
        audiobook_normalize_utf8(description);

        /* Choose final author/narrator/series display strings. */
        int force_path = (path_author[0] == '_');
        const char *clean_path_author = force_path ? (path_author + 1) : path_author;
        
        const char *author_name = force_path ? clean_path_author 
                                             : (tag_artist[0] ? tag_artist : path_author);
        /* If forcing path, we still allow narrator from ID3 since path doesn't provide it */
                ...
                if (parsed <= 0) {
                    if (bd->file_count > 1) {
                        ...
                        for (int c = 0; c < nch; c++) {
                            char ch_title[512];
                            if (nch == 1) {
                                snprintf(ch_title, sizeof(ch_title), "%s", track_title);
                            } else {
                                snprintf(ch_title, sizeof(ch_title), "Chapter %d", c + 1);
                            }
...
int audiobook_cleanup_orphans(sqlite3 *db, scan_progress_cb progress,
    ...
    /* Clean up orphaned authors and series */
    sqlite3_exec(db, "DELETE FROM authors WHERE author_id NOT IN (SELECT DISTINCT author_id FROM books)", NULL, NULL, NULL);
    sqlite3_exec(db, "DELETE FROM series WHERE series_id NOT IN (SELECT DISTINCT series_id FROM books)", NULL, NULL, NULL);
